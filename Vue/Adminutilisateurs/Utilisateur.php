...
<article>
    <header>
        <h1 class="titreUtilisateur">Informations pour l'utilisateur #<?= $this->nettoyer($utilisateurEntree['id']) ?></h1>
        <hr />
    </header>
...
<h2 id="titreUtilisateurVehicule">Voir les véhicules de cet utilisateur</h2>
    <ul>
        <?php foreach($vehicules as $vehicule): ?>
            <a href="Adminvehicules/vehicule/<?= $vehicule['id'] ?>"><li>-> #<?= $this->nettoyer($vehicule['id']) ?>: <?= $this->nettoyer($vehicule['marque']) ?> <?= $this->nettoyer($vehicule['modele']) ?> <-</li></a>
        <?php endforeach ?>
    </ul>
